...
import { notNull } from 'src/utils/database';
...
const withOwner = (eb: ExpressionBuilder<DB, 'albums'>) => {
  return notNull(
    jsonObjectFrom(eb.selectFrom('users').select(userColumns).whereRef('users.id', '=', 'albums.ownerId')),
  ).as('owner');
};
...
const withAlbumUsers = (eb: ExpressionBuilder<DB, 'albums'>) => {
  return jsonArrayFrom(
    eb
      ...
      .select((eb) =>
        notNull(
          jsonObjectFrom(eb.selectFrom('users').select(userColumns).whereRef('users.id', '=', 'album_users.usersId')),
        ).as('user'),
      )
...
const withAssets = (eb: ExpressionBuilder<DB, 'albums'>) => {
  return eb
    ...
    .select((eb) => eb.fn.jsonAgg('asset').$castTo<Asset[]>().as('assets'))
    .as('assets');
};
